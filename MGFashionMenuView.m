...
static CAShapeLayer * layerMask = nil;
...
- (instancetype)initWithMenuView:(UIView *)menuView {
    return [self initWithMenuView:menuView withStyle:2];
}

//Init the menuView and the helperViews
- (instancetype)initWithMenuView:(UIView *)menuView withStyle:(NSInteger)bannerStyle {
    ...
    if (self = [super initWithFrame:fakeRect]) {
        ...
        //Animation
        if(style == 0 || style == 3) //Soft Bounce
            [self mg_setValuesForAnimationType:MGAnimationTypeSoftBounce];
        else if(style == 1 || style == 4) //Hard Bounce
            [self mg_setValuesForAnimationType:MGAnimationTypeHardBounce];
        else if(style == 2)
            [self mg_setValuesForAnimationType:MGAnimationTypeBounce];
        else if(style == 5)
            [self mg_setValuesForAnimationType:MGAnimationTypeWave];
...
- (void)drawRect:(CGRect)rect
{
    if (self.subviews.count == 0) {
       ...
       layerMask = [CAShapeLayer layer];
        layerMask.fillColor = [UIColor blackColor].CGColor;
        
        if(style != 0 && style != 1)
            _menuView.layer.mask = layerMask;
    }
    ...
    //NSLog(@"Path rect: %@", NSStringFromCGRect(CGPathGetPathBoundingBox(path.CGPath)));
    layerMask.path = path.CGPath;
    //layerMask.fillRule = kCAFillRuleEvenOdd;         // this line is new
